...
# Iterates through KAN architectures in parallel.
# Usage:
#   bash scripts/run_yolo_kan_variants.sh --freeze
#   bash scripts/run_yolo_kan_variants.sh --unfreeze
# ─────────────────────────────────────────────────────────────────────────────
...
if [ "$1" == "--freeze" ]; then
    ...
    echo "Please specify --freeze or --unfreeze as the first argument."
    echo "Example: bash scripts/run_yolo_kan_variants.sh --freeze"
    exit 1
fi
...
# Wait for all background instances to finish simultaneously
wait
